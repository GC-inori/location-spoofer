...
FAV_MANAGE="$ROOT/App/FavoriteManagementView.swift"
MAIN_CONTAINER="$ROOT/App/MainContainerView.swift"
SEARCH_COMPLETER="$ROOT/App/LocationSearchCompleter.swift"
SETTINGS="$ROOT/App/SettingsView.swift"

for file in "$MAP_HOME" "$MAP_STATE" "$MAP_BRIDGE" "$REALTIME" "$SETUP" "$PROXY" "$SETTINGS_NAVIGATOR" "$DIAGNOSTICS" "$CONTENT" "$CONVERTER" "$NETWORK_MONITOR" "$FAV_MANAGE" "$MAIN_CONTAINER" "$SEARCH_COMPLETER" "$SETTINGS"; do
...
grep -q 'showsCompass = false' "$MAP_BRIDGE" || fail "default MapKit compass must be hidden so it does not overlap the menu"
! grep -q 'onZoomIn' "$MAP_BRIDGE" || fail "map bridge must not expose zoom-in buttons"
! grep -q 'onZoomOut' "$MAP_BRIDGE" || fail "map bridge must not expose zoom-out buttons"
! grep -q 'zoomInTapped' "$MAP_BRIDGE" || fail "map bridge must not keep zoom-in button handlers"
! grep -q 'zoomOutTapped' "$MAP_BRIDGE" || fail "map bridge must not keep zoom-out button handlers"
! grep -q 'onZoomIn' "$MAP_HOME" || fail "map home must not wire zoom-in buttons"
! grep -q 'onZoomOut' "$MAP_HOME" || fail "map home must not wire zoom-out buttons"
...
grep -q 'SelectedLocationAnnotationView' "$MAP_BRIDGE" || fail "map must use the compact custom pin annotation view"
grep -q 'LocationSearchCompleter' "$SEARCH_COMPLETER" || fail "search completer must be available"
grep -q 'favoriteButton' "$MAP_HOME" || fail "map home must expose the quick favorite button"
grep -q 'recentFavoritesSection' "$MAP_HOME" || fail "map home must expose recent favorites section"
grep -q 'coordinateRow(title: "国内标准 (GCJ-02)"' "$FAV_MANAGE" || fail "favorite detail must label domestic coordinate as GCJ-02"
grep -q 'coordinateRow(title: "国际标准 (WGS-84)"' "$FAV_MANAGE" || fail "favorite detail must label international coordinate as WGS-84"
grep -q 'phase = .map' "$CONTENT" || fail "ContentView must explicitly gate Map construction"
...
grep -q 'activeTip = .deactivation' "$SETTINGS" || fail "manual deactivation help must use the non-suppressible generic tip sheet"
